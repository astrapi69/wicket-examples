...
public class PopupoverlaySettings implements Serializable {
	...
	/**
	 * Disable background cover and keep background elements active. Implies
	 * background, blur and scrolllock to false; defaults to false;
	 */
	...
	/**
	 * Sets horizontal position. Options `leftedge` and `rightedge` can be used
	 * only for tooltips, and will align the tooltip to the left or right edge
	 * of the opening element (`openelement`). defaults to 'center';
	 */
	 ...
	 * 
	 * Options `topedge` and `bottomedge` can be used only for tooltips, and
	 * will align the tooltip to the top or bottom edge of the opening element
	 * (`openelement`). defaults to 'center';
	 */
	...
	/**
	 * Lock keyboard focus inside of popup. Recommended to be enabled; defaults
	 * to true;
	 */
	private boolean keepfocus = true;

	/**
	 * Enables you to specify the element which will be focused upon showing the
	 * popup. By default, the popup element #my_popup will receive the initial
	 * focus. defaults to 'my_popup';
	 */
	private String focuselement = null;

	/**
	 * Sets a delay in milliseconds before focusing an element. This is to
	 * prevent page scrolling during opening transition, as browsers will try to
	 * move the viewport to an element which received the focus. (defaults to
	 * 50)
	 */
	private int focusdelay = 50;

	/**
	 * Sets a page container (to help screen reader users). Page container
	 * should be the element that surrounds all the content on the page (e.g.
	 * '.container' in the case of this very page).
	 * 
	 * It's highly recommended that you set the page container to help some
	 * screen readers read the modal dialog correctly. Doing so, when the popup
	 * is visible, aria-hidden="true" will be set to the page container and
	 * aria-hidden="false" to the popup, and vice-versa when the popup closes.
	 * You can set `pagecontainer` once per website (e.g.
	 ...
	 * 
	 * Setting to false is equivalent to #my_popup{outline: none;}; defaults to
	 * false;
	 */
	private boolean outline = false;

	/**
	 * Removes popup element from the DOM after closing transition.; defaults to
	 * false;
	 */
	 ...
	 * 
	 * Use this if you don't need separate transition for background, or
	 * different transition for opening and closing the popup, or if you need to
	 * transition only selected properties – otherwise set custom transitions
	 * directly in CSS.
	 * 
	 * Simple fade effect $('#my_popup').popup({transition: 'all 0.3s'}) is
	 * equivalent to #my_popup, #my_popup_wrapper, #my_popup_background
	 * {transition: all 0.3s;}
	 * 
	 * Setting fade effect for all popups on the site:
	 * $.fn.popup.defaults.transition = 'all 0.3s'; is equivalent to
	 ...
	 * 
	 * For example, for multiple opening links using the same tooltip on
	 * mouseover:
	 * 
	 * $('.my_popup_open').on({ mouseenter: function(event) {
	 * $('#my_popup').popup({ tooltipanchor: event.target, autoopen: true, type:
	 * 'tooltip' }); }, mouseleave: function() { $('#my_popup').popup('hide'); }
	 * });
	 */
